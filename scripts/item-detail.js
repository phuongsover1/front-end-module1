const itemIdInLocal = localStorage.getItem('item-shop-id');
const itemId = itemIdInLocal ?? '';
const itemNameH6 = document.getElementById('item-name');
const moneyDiv = document.getElementById('money-div');
const colorCircus = document.getElementById('color-circus');
const colorName = document.getElementById('color-name');
const itemDetailCarousel = document.getElementById('item-detail-carousel');
const detailImg = document.getElementById('detail-img');

const items = JSON.parse(localStorage.getItem('items'));
const itemWithId = items.find(item => item.id === +itemId);
console.log(itemWithId);

const carouselContent = itemWithId.image.detailImg.reduce(
  (content, currentImg) => {
    return (content += `<img src='${currentImg}'/>`);
  },
  ``
);
itemDetailCarousel.innerHTML = carouselContent;

itemNameH6.textContent = `${itemWithId.name}`;

moneyDiv.innerHTML = `C$ &nbsp;&nbsp;${itemWithId.price}`;
colorName.innerText = itemWithId.colorName;
colorCircus.style.backgroundColor = itemWithId.color;

detailImg.src = `${itemWithId.image.smallImg}`;
...
//lay item và ảnh trong localStorage
//
$('.item-detail-carousel').slick({
  slidesToShow: 2,
  slidesToScroll: 1,
